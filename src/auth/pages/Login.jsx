...
const keychainConfig = [
  ...
  { text: "JS",      color: "#EDFF2C", shape: "hexagon",    textColor: "#000000", spawnAngle: 305 },
  { text: "NEXT.JS", color: "#ffffff", shape: "octahedron", textColor: "#000000", spawnAngle: 340 },
];

const params = {
  forceMultiplier: 3.0, glassTransmission: 1.0, glassRoughness: 0.15,
  glassThickness: 0.8,  glassIor: 1.5,          ringColor: "#d4d4d8", ambientLight: 0.5,
};
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    sd.physicsObjects = []; sd.interactableMeshes = [];
    ...
    const sharedGlassMaterial = new THREE.MeshPhysicalMaterial({
      metalness: 0.1, roughness: params.glassRoughness, transmission: params.glassTransmission,
      thickness: params.glassThickness, ior: params.glassIor, transparent: true, side: THREE.DoubleSide,
    });
    ...
    physicsWorld.addContactMaterial(new CANNON.ContactMaterial(keychainMat, keychainMat, {
    ...
    function buildMainRing() {
      ...
      const ringMesh = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: params.ringColor, metalness: 1.0, roughness: 0.2 })
      );
      scene.add(ringMesh);

      const ringBody = new CANNON.Body({
        mass: 40.0, type: CANNON.Body.DYNAMIC,
      ...
      for (let i = 0; i < 36; i++) {
        ...
        ringBody.addShape(
          new CANNON.Box(new CANNON.Vec3(arcLen * 0.55, tubeRadius * 1.5, tubeRadius * 1.5)),
          new CANNON.Vec3(ringRadius * Math.cos(angle), ringRadius * Math.sin(angle), 0), q
        );
      }
      ...
      physicsWorld.addConstraint(new CANNON.PointToPointConstraint(
        ringBody, new CANNON.Vec3(0, ringRadius, 0), topPivot, new CANNON.Vec3(0, 0, 0)
      ));
    ...
    function buildKeychains() {
      ...
      keychainConfig.forEach((skill) => {
        ...
        dummyArm.collisionFilterGroup = GROUP_DUMMY; dummyArm.collisionFilterMask = 0;
        ...
        const holeMesh = new THREE.Mesh(
          ...
          new THREE.MeshStandardMaterial({ color: "#d4d4d8", metalness: 1, roughness: 0.2 })
        );
        scene.add(holeMesh);
        ...
        const localX   = new THREE.Vector3().crossVectors(localY, tangent).normalize();
        const smallRingQuat = new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(localX, localY, tangent));

        const smallRingBody = new CANNON.Body({
          mass: 1.0, position: smallRingPos, shape: new CANNON.Sphere(smallRingRadius),
          linearDamping: 0.01, angularDamping: 0.2,
        });
        smallRingBody.collisionFilterGroup = GROUP_SMALL_RING; smallRingBody.collisionFilterMask = 0;
        ...
        sd.physicsObjects.push({ mesh: holeMesh, body: smallRingBody, isSmallRing: true });
        ...
        const mat = sharedGlassMaterial.clone(); mat.color.set(skill.color);
        const glassMesh = new THREE.Mesh(geometry, mat);
        keychainGroup.add(glassMesh);
        sd.interactableMeshes.push(glassMesh);

        const textMat = new THREE.MeshBasicMaterial({ map: createTextTexture(skill.text, skill.textColor), transparent: true, depthWrite: false });
        ...
        textBack.position.z = -(depth/2) - 0.01; textBack.rotation.y = Math.PI;
        keychainGroup.add(textBack);

        const pivotA_local = new CANNON.Vec3(0, -smallRingRadius, 0);
        const pivotA_world = new THREE.Vector3(pivotA_local.x, pivotA_local.y, pivotA_local.z).applyQuaternion(smallRingQuat).add(new THREE.Vector3(smallRingPos.x, smallRingPos.y, smallRingPos.z));
        ...
        const keychainBody = new CANNON.Body({ mass: 1.5, material: scene.userData.keychainMaterial, shape: shapePhysics, position: spawnPos, linearDamping: 0.3, angularDamping: 0.5 });
    ...
    function onMouseMove(e) {
      ...
      const velX = mouse.x - lastMousePos.x, velY = mouse.y - lastMousePos.y;
      lastMousePos.copy(mouse);
      raycaster.setFromCamera(mouse, camera);
      const hits = raycaster.intersectObjects(sd.interactableMeshes, false);
      if (hits.length > 0) {
        const hit = hits[0], body = hit.object.userData.physicsBody;
        if (body) {
          const clamp = (v, m) => Math.max(Math.min(v, m), -m);
          const cVX = clamp(velX, 0.15), cVY = clamp(velY, 0.15);
          const forceMag = params.forceMultiplier * 4 * body.mass;
          const impulse = new CANNON.Vec3(cVX * forceMag, cVY * forceMag, -Math.abs(cVX + cVY) * forceMag * 0.8);
          if (Math.abs(cVX) < 0.001 && Math.abs(cVY) < 0.001)
            impulse.set((Math.random()-.5)*body.mass*0.2, (Math.random()-.5)*body.mass*0.2, -0.2*body.mass);
    ...
    function animate() {
      ...
      sd.physicsObjects.forEach((obj) => {
        if (obj.isSmallRing && ringBody) {
          ...
          const lt = new CANNON.Vec3(-localPos.y, localPos.x, 0); lt.normalize();
          ...
          obj.body.applyTorque(new CANNON.Vec3(torque.x*20-av.x*2, torque.y*20-av.y*2, torque.z*20-av.z*2));
        }
      ...
      <div className="login-root">
        ...
        <div className="form-pane">
          <div className="form-card">
            ...
            <h1 className="form-title">Sign in to<br />your workspace</h1>
